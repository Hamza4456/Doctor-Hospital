/* =====================================================
   ...
   - Forced auth popup on index.html (cannot be dismissed
     until the visitor signs in or creates an account)
   - Admin pages are protected (staff session required)
   - Dashboard is shown INSIDE a dropdown (staff chip),
     and also inside the patient dropdown when both are
...
(function () {
  ...
  /* =====================================================
     SECTION 0 — VIEWPORT META SAFEGUARD (mobile fix)
     If the page forgot <meta name="viewport"> the whole
     site renders at ~980px wide on phones and every media
     query below looks broken. Inject it if it's missing.
  ===================================================== */
  ...
  /* =====================================================
     SECTION A — PATIENT AUTH (built-in)
  ===================================================== */
  const UserAuth = (function () {
    const DB_KEY = 'medicare_users';
    const SESSION_KEY = 'medicare_user_session';        // ← patient session
  ...
  /* =====================================================
     ...
     Uses the SAME key that dashboard.html uses:
     sessionStorage['medicare_session']
     → user session and staff session are completely separate
  ===================================================== */
  const StaffAuth = {
  ...
  // Dashboard / admin pages require a valid STAFF session.
  // If not signed in as staff → bounce to the staff login page.
  if (ADMIN_PAGES.indexOf(path) !== -1) {
    if (!StaffAuth.isLoggedIn()) {
      location.replace('admin-login.html');
    }
    return;   // never inject shared header/footer into admin pages
  }
  ...
  if (!document.querySelector('style[data-shared-css]')) {
    ...
    css.textContent = `
      /* ============================================================
         STICKY LAYOUT (header + footer)
         - Header sticks to top of viewport
         - Footer sticks to bottom of page / viewport
      ============================================================ */
      html { height: 100%; }
      ...
      /* Everything except the footer keeps natural height */
      ...
      /* ===== NAVBAR ===== */
      .navbar { background: #fff; box-shadow: 0 2px 20px rgba(0,30,45,0.05); }
      .navbar-inner { display: flex; align-items: center; justify-content: space-between; padding: 18px 0; gap: 20px; }
      .nav-logo { display: flex; align-items: center; gap: 12px; flex-shrink: 0; }
      .nav-logo .logo-icon { background: var(--primary, #0d6e9e); color: #fff; width: 46px; height: 46px; border-radius: 13px; display: flex; align-items: center; justify-content: center; font-size: 22px; box-shadow: 0 8px 16px -4px rgba(13,110,158,0.3); }
      .nav-logo h1 { font-size: 20px; font-weight: 800; color: var(--dark, #0b3b4b); line-height: 1; }
      .nav-logo p { font-size: 11px; color: var(--gray, #5e7e8c); font-weight: 500; margin-top: 3px; }
      .nav-links { display: flex; align-items: center; gap: 8px; list-style: none; }
      .nav-links a { padding: 10px 18px; border-radius: 10px; font-size: 14px; font-weight: 600; color: var(--gray, #5e7e8c); transition: all 0.15s; }
      ...
      .nav-staff-chip .dot {
        ...
        background: #c96f1e;
        box-shadow: 0 0 0 3px rgba(201,111,30,0.2);
      }
      .nav-staff-chip .chev { font-size: 9px; margin-left: 3px; opacity: 0.75; }
      .nav-staff-menu {
      ...
      .nav-staff-menu a {
        ...
        color: var(--dark, #0b3b4b); cursor: pointer;
      }
      ...
      .nav-user-chip { display: flex; align-items: center; gap: 10px; background: #fff; border: 1.5px solid var(--border, #e6f0f5); border-radius: 40px; padding: 5px 14px 5px 5px; cursor: pointer; transition: all 0.18s; user-select: none; }
      .nav-user-chip:hover { border-color: var(--primary, #0d6e9e); background: #f9fcfd; }
      .nav-user-avatar { width: 34px; height: 34px; border-radius: 50%; background: linear-gradient(135deg, #0d6e9e, #0e8b5e); color: #fff; display: flex; align-items: center; justify-content: center; font-weight: 700; font-size: 12.5px; flex-shrink: 0; }
      ...
      .nav-user-menu { position: absolute; top: calc(100% + 8px); right: 0; min-width: 230px; background: #fff; border-radius: 16px; padding: 8px; border: 1px solid var(--border, #e6f0f5); box-shadow: 0 20px 50px -12px rgba(0,80,110,0.2); opacity: 0; visibility: hidden; transform: translateY(-6px); transition: all 0.18s; z-index: 200; }
      .nav-user-menu.open { opacity: 1; visibility: visible; transform: translateY(0); }
      .nav-user-menu a { display: flex; align-items: center; gap: 10px; padding: 10px 14px; border-radius: 10px; font-size: 13.5px; font-weight: 500; color: var(--dark, #0b3b4b); cursor: pointer; }
      ...
      /* ===== BUTTONS ===== */
      .btn { padding: 12px 22px; border-radius: 12px; border: none; font-family: inherit; font-size: 14px; font-weight: 600; cursor: pointer; transition: all 0.2s; display: inline-flex; align-items: center; justify-content: center; gap: 8px; text-decoration: none; white-space: nowrap; }
      ...
      /* ===== FOOTER ===== */
      .footer { background: #0b3b4b; color: #c5d8e2; padding: 60px 0 24px; }
      .footer-grid { display: grid; grid-template-columns: 2fr 1fr 1fr 1.4fr; gap: 40px; margin-bottom: 40px; }
      .footer-brand { display: flex; align-items: center; gap: 12px; margin-bottom: 20px; }
      .footer-brand .logo-icon { background: var(--primary, #0d6e9e); color: #fff; width: 44px; height: 44px; border-radius: 13px; display: flex; align-items: center; justify-content: center; font-size: 22px; }
      .footer-brand h1 { font-size: 18px; color: #fff; font-weight: 800; }
      .footer-brand p { font-size: 11px; opacity: 0.7; margin-top: 2px; }
      ...
      .footer-links a { font-size: 14px; opacity: 0.75; transition: all 0.15s; }
      .footer-links a:hover { opacity: 1; color: #7dd3fc; padding-left: 4px; }
      .footer-contact-item { display: flex; gap: 12px; margin-bottom: 14px; font-size: 14px; opacity: 0.85; }
      .footer-contact-item i { color: #7dd3fc; margin-top: 4px; width: 16px; }
      .footer-bottom { border-top: 1px solid rgba(255,255,255,0.1); padding-top: 24px; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 16px; font-size: 13px; opacity: 0.7; }
      .footer-socials { display: flex; gap: 12px; }
      .footer-socials a { width: 38px; height: 38px; border-radius: 10px; background: rgba(255,255,255,0.08); display: flex; align-items: center; justify-content: center; transition: all 0.15s; }
      ...
      .auth-popup-hero::before { content: ''; position: absolute; width: 250px; height: 250px; background: rgba(255,255,255,0.08); border-radius: 50%; top: -100px; right: -80px; }
      .auth-popup-hero::after { content: ''; position: absolute; width: 180px; height: 180px; background: rgba(255,255,255,0.05); border-radius: 50%; bottom: -80px; left: -60px; }
      .auth-popup-logo {
      ...
      .auth-popup-hero h2 { font-size: 22px; font-weight: 800; margin-bottom: 6px; letter-spacing: -0.4px; position: relative; z-index: 2; }
      .auth-popup-hero p { font-size: 13.5px; opacity: 0.85; max-width: 320px; margin: 0 auto; position: relative; z-index: 2; }
      .auth-popup-tabs { display: flex; padding: 20px 28px 0; gap: 4px; background: #fff; border-bottom: 1px solid #e6f0f5; position: relative; }
      .auth-popup-tab {
      ...
      .auth-popup-body { padding: 24px 28px 28px; }
      ...
      .auth-popup .form-group label { display: block; font-size: 12px; font-weight: 700; color: #0b3b4b; margin-bottom: 6px; }
      .auth-popup .form-group input {
        width: 100%; padding: 12px 14px;
        border: 1.5px solid #e6f0f5; border-radius: 12px;
      ...
      .auth-popup .pass-wrap input { padding-right: 46px; }
      ...
      .auth-popup .btn { display: inline-flex; align-items: center; justify-content: center; gap: 8px; padding: 13px 22px; border-radius: 12px; border: none; font-family: inherit; font-size: 14px; font-weight: 700; cursor: pointer; text-decoration: none; transition: all 0.2s; }
      ...
      .auth-popup .terms { display: flex; align-items: flex-start; gap: 9px; margin: 12px 0 16px; font-size: 12.5px; color: #5e7e8c; line-height: 1.5; }
      .auth-popup .terms input { width: auto; margin-top: 3px; accent-color: #0d6e9e; }
      .auth-popup .strength { height: 5px; border-radius: 5px; background: #e6f0f5; margin-top: 8px; overflow: hidden; }
      ...
      .auth-popup-foot { text-align: center; font-size: 12.5px; color: #5e7e8c; margin-top: 14px; padding-top: 14px; border-top: 1px solid #e6f0f5; }
      ...
      /* -------- BASE (desktop) helpers for the mobile nav -------- */
      .navbar { position: relative; }         /* anchor for the mobile panel */
      .nav-toggle { display: none; }          /* only visible on mobile   */
      .nav-collapse {
        display: flex;
        align-items: center;
        gap: 8px;
      ...
      .nav-links { margin: 0 auto; }          /* keeps links visually centred */

      /* -------- POPUP SCROLL FIX (short viewports) --------
         With align-items:center + overflow-y:auto the top of a
         tall popup becomes unreachable. flex-start + margin:auto
         centres it when it fits and top-aligns it when it doesn't. */
      .auth-popup-overlay { align-items: flex-start; }
      .auth-popup { margin: auto; }
      ...
      @media (max-width: 1024px) {
        ...
        .nav-user-chip { padding: 5px; gap: 0; }
        .nav-user-chip .chev { display: none; }
      }

      /* ================= MOBILE NAV (≤960px) ================= */
      @media (max-width: 960px) {
        .navbar-inner { padding: 12px 0; gap: 12px; }
        .nav-logo { min-width: 0; gap: 10px; }
        .nav-logo .logo-icon { width: 40px; height: 40px; font-size: 19px; border-radius: 11px; }
        .nav-logo h1 { font-size: 18px; }
        .nav-logo p { font-size: 10px; }
        ...
        .nav-collapse {
          ...
          box-shadow: 0 24px 40px -18px rgba(0,60,90,0.28);
        ...
        /* dropdowns become inline accordions (no more clipping) */
        .nav-staff-menu,
        .nav-user-menu {
          position: static;
          opacity: 1;
        ...
        .nav-staff-menu a,
        .nav-user-menu a { padding: 12px 14px; font-size: 14.5px; }
      }
      ...
      @media (max-width: 520px) {
        .auth-popup-overlay { padding: 12px; }
        .auth-popup { border-radius: 20px; }
        .auth-popup-hero { padding: 26px 22px 22px; }
        .auth-popup-hero h2 { font-size: 19px; }
        .auth-popup-tabs { padding: 16px 22px 0; }
        .auth-popup-body { padding: 20px 22px 24px; }
        .auth-popup .form-row { grid-template-columns: 1fr; gap: 0; }

        /* 16px stops iOS from zooming the whole page on focus */
      ...
      @media (max-height: 640px) {
        .auth-popup-hero { padding: 20px 22px 16px; }
        .auth-popup-hero h2 { font-size: 18px; }
        .auth-popup-hero p { display: none; }
        .auth-popup-logo { width: 44px; height: 44px; font-size: 20px; margin-bottom: 10px; }
      }
  ...
  /* =====================================================
     ...
     Staff chip (with Dashboard dropdown) + patient chip
  ===================================================== */
  ...
  function bindNavToggle() {
    ...
    if (toggle.dataset.bound === '1') return;   // don't double-bind
    ...
    // close the panel when a real link inside it is tapped
  ...
  // Reset the panel state when resizing up to desktop
  ...
  // Global click listener — closes menus (and the mobile panel) when clicking outside
  ...
  function renderAuthSlot() {
    ...
    /* ---------- STAFF AREA ---------- */
    if (isStaff) {
      // Staff is logged in → show "Staff" chip that is a dropdown
      // with Dashboard + Staff Logout inside.
      html += `
        <div class="nav-staff">
      ...
      // Staff not logged in → show staff login link
    ...
    // Logged in as patient → show chip + dropdown
    // If a staff session also exists in this tab, surface a "Staff Dashboard"
    // entry inside the patient dropdown too.
    html += `
  ...
  function isAuthLocked() {
    // Popup cannot be dismissed on the home page when the visitor
    // has not yet signed in / created a patient account.
  ...
  function openAuthPopup(tab) {
    ...
    closeNavCollapse();      // make sure the mobile panel isn't covering it
    closeAllMenus();

    overlay.classList.add('show');
    document.body.style.overflow = 'hidden';

    // Hide the close button when auth is mandatory on this page
  ...
  function closeAuthPopup(force) {
    const overlay = document.getElementById('authPopupOverlay');
    if (!overlay) return;
    if (!force && isAuthLocked()) return;   // locked on home page
  ...
  function bindPopupEvents() {
    ...
    document.getElementById('popupLoginForm').addEventListener('submit', async e => {
      ...
      closeAuthPopup(true);   // force close (bypass lock)
      location.reload();
    });
  ...
  function injectPopup() {
    ...
    // Auto-show on home page when not signed in.
    if (isAuthLocked()) {
  ...
  // Expose so pages can refresh the chip after login/profile updates
